...
int best_fit(struct mm memory_manager[],int size,int index,int n);
void partitioning(struct mm memory_manager[],int n);
int deallocate(struct mm memory_manager[],int index,int n);
void output(struct mm memory_managerp[],int n);
int main()
{   int size,n,choice=1,allocsize,index=1,k,choice2,dealloc;
	...
	memory_manager[0].start_block=(char *)malloc(size);
 	...
 	while(choice)
 	{
 		...
 		if(choice2==1)
 		{scanf("%d",&allocsize);
 		k=best_fit(memory_manager,allocsize,index,n);
 		if(k)
...
int best_fit(struct mm memory_manager[],int size,int index,int n)
{ int i,j=-1,minmem=10000000,no=-1;
	...
	for (int i = 0; i <n; i++)
	{
		...
		temp=memory_manager[i].header;
		if(temp!=NULL && temp->start -memory_manager[i].start_block >=size &&  temp->start -memory_manager[i].start_block)
		{minmem=memory_manager[i].unused;
			mini=memory_manager[i].header;
			no=i;
			j=1;
